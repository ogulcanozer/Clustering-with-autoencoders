...
class make_ae:
    ...
    def Encoder(self, f_dim, h_dims, activs):
        ...
        self.encode = Dense (h_dims[0],kernel_regularizer=regularizers.l2(0.01),activation = activs[0])(f_dim)
        if(len(h_dims)>1):
    ...
    def __init__(self, f_dim, e_dims, o_dim ,d_dims, e_acts, d_acts):
        self.input_e = Input(shape=(f_dim,), dtype='float32', name='main_input')
        self.encode = self.Encoder(self.input_e,e_dims,e_acts)
        input_d = self.encode
        self.decode = self.Decoder(input_d,d_dims,d_acts)
        self.encoder = Model(input=self.input_e, output=[self.encode])
        self.autoencoder = Model(inputs=self.input_e,outputs=[self.decode])
